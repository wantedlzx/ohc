...
public final class OHCacheChunkedImpl<K, V> implements OHCache<K, V>
{
    ...
    {
        putInternal(key, value, false, null, OHCache.USE_DEFAULT_EXPIRE_AT);
    }
    ...
    {
        return putInternal(key, value, false, old, OHCache.USE_DEFAULT_EXPIRE_AT);
    }
    ...
    {
        return putInternal(key, value, true, null, OHCache.USE_DEFAULT_EXPIRE_AT);
    }
    ...
    private boolean putInternal(K k, V v, boolean ifAbsent, V old, long expireAt)
    {
        ...
        if (isFixedSize())
            return putInternalFixed(k, v, ifAbsent, old, expireAt);

        return putInternalVariable(k, v, ifAbsent, old, expireAt);
    }

    private boolean putInternalFixed(K k, V v, boolean ifAbsent, V old, long expireAt)
    {
    ...
    private boolean putInternalVariable(K k, V v, boolean ifAbsent, V old, long expireAt)
    {
